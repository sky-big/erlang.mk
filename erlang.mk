...
# MAKEFILE_LIST环境变量是包括环境变量"MAKEFILES"指定，命令行指，当前工作下的默认的以及使用指示符"include"
# 指定包含的，在对这些文件金喜解析执行之前make读取的文件名将会自动的追加到该变量中，注意"include"包含的肯定
# 是在最后面
# realpath函数是得到文件的绝对路径
# lastword函数是获得变量中的最后一个词
ERLANG_MK_FILENAME := $(realpath $(lastword $(MAKEFILE_LIST)))

ERLANG_MK_VERSION = 2.0.0-pre.2-140-gc313f4d-dirty


# Core configuration.
# 如果没有定义PROJECT变量则将当前目录作为项目名字
PROJECT ?= $(notdir $(CURDIR))
PROJECT := $(strip $(PROJECT))

# 如果没有定义项目版本号则默认定义为rolling
PROJECT_VERSION ?= rolling
PROJECT_MOD ?= $(PROJECT)_app


# Verbosity.
# 定义当前makefile文件命令执行的前缀
V ?= 0
...
# 搜索erlang相关的项目(使用的时候是make search q=搜索内容)
search:
ifdef q
	$(foreach p,$(PACKAGES), \
...
# 如果用户需要特别指出依赖项的源则需要定义变量dep_依赖项的名字=源,如果没有定义该变量则erlang.mk脚本从自己的开源包列表里查询得到源
dep_repo = $(patsubst git://github.com/%,https://github.com/%, \
	$(if $(dep_$(1)),$(word 2,$(dep_$(1))),$(pkg_$(1)_repo)))

# 首先使用erlang.mk脚本中依赖项的版本号，如果没有则从用户自己定义的变量中的第三个单词得到
...
apps:: $(ALL_APPS_DIRS)
	@: 如果IS_APP IS_DEP都是空则将apps.log文件删除重新生成
ifeq ($(IS_APP)$(IS_DEP),)
...
# fix, a proper fix would be to compile apps/* in the right order.
	@: 确保所有apps目录下各个应用的ebin目录的存在
	$(verbose) for dep in $(ALL_APPS_DIRS) ; do \
		mkdir -p $$dep/ebin || exit $$?; \
	done
	@: 进入apps目录下的每个应用目录进行make操作
	$(verbose) for dep in $(ALL_APPS_DIRS) ; do \
...
deps:: $(ALL_DEPS_DIRS) apps
	@: 如果IS_APP IS_DEP都是空则将deps.log文件删除重新生成
ifeq ($(IS_APP)$(IS_DEP),)
...
define dep_autopatch
	@: 如果依赖项中有erlang.mk则将erlang.mk脚本中的内容替换为包含本项目的erlang.mk脚本
	if [ -f $(DEPS_DIR)/$(1)/erlang.mk ]; then \
		# 将依赖项的app.src文件中的modules,vsn,registered的对应信息清空
		$(call erlang,$(call dep_autopatch_appsrc.erl,$(1))); \
		# 将依赖项中的erlang.mk脚本中的内容替换为包含本项目中的erlang.mk脚本
	...
	@: 如果依赖项中有Makefile文件
		...
		if [ 0 != `grep -c "include ../\w*\.mk" $(DEPS_DIR)/$(1)/Makefile` ]; then \
			...
			@: 扫描依赖项src目录中的所有.erl文件得到依赖项的所有模块列表然后写入到依赖项ebin目录下的.app文件中
			$(call erlang,$(call dep_autopatch_app.erl,$(1))); \
		fi \
	else \
		if [ ! -d $(DEPS_DIR)/$(1)/src/ ]; then \
			@: 对没有Makefile的依赖项生成一个空的Makefile
...
# shell脚本中if判断语句中-f判断是否是文件,-o表示或,-a表示与
# 将依赖项.app.src.script文件中的内容写入到.app.src文件中,然后将.app.src文件中的modules等相关信息清空，
# 然后如果依赖项有rebar相关的信息，则根据rebar.config中的信息生成新的Makefile文件(先得到rebar的项目放入到
# 临时目录中),如果没有rebar相关的信息则生成默认的Makefile文件
define dep_autopatch2
	@: 将.app.src.script文件中的信息写入到.app.src文件中
	...
	if [ -f $(DEPS_DIR)/$(1)/rebar -o -f $(DEPS_DIR)/$(1)/rebar.config -o -f $(DEPS_DIR)/$(1)/rebar.config.script ]; then \
		@: 在.erlang.mk临时目录里得到rebar项目
		$(call dep_autopatch_fetch_rebar); \
		@: 将依赖项中老的Makefile文件备份然后根据rebar.config文件生成新的Makefile文件
		$(call dep_autopatch_fetch_rebar,$(1)); \
	else \
		@: 如果依赖项没有rebar相关的信息,则主动给依赖项生成Makefile文件，里面的内容加上编译debug参数以及
		@: 包含本项目的erlang.mk脚本
...
define dep_autopatch_rebar
	@: 如果依赖项有Makefile文件则将该文件备份到Makefile.orig.mk文件中
	if [ -f $(DEPS_DIR)/$(1)/Makefile ]; then \
		mv $(DEPS_DIR)/$(1)/Makefile $(DEPS_DIR)/$(1)/Makefile.orig.mk; \
	fi; \
	@: 在没有Makefile但是拥有rebar.config的依赖项中,根据rebar.config中的信息生成依赖项的Makefile文件
	$(call erlang,$(call dep_autopatch_rebar.erl,$(1))); \
	@: 将依赖项中ebin目录中的.app文件删除掉
...
# 取得依赖项获取的方式,比如git等等(先从用户自定义的依赖项中的第一个单词去取源的获取方式)
define dep_fetch
	$(if $(dep_$(1)), \
...
$(DEPS_DIR)/$(call dep_name,$1):
	...
	@: 先根据dep_fetch命令包得到获取依赖项的方式，然后调用不同的获取依赖项的命令包
	$(dep_verbose) $(call dep_fetch_$(strip $(call dep_fetch,$(1))),$(1))
	@: 下面是查看该依赖项下载下来之后是否有configure文件，如果有的话立刻执行
...
ifeq ($(filter $(1),$(NO_AUTOPATCH)),)
	@: 如果是rabbitmq相关的依赖项需要下载rabbitmq协议源代码生成的工具
...
# -include忽略由于包含文件不存在或者无法创建时的错误提示
-include $(DEPS_DIR)/$(1)
...
# --no-print-directory:表示不要在屏幕上打印"Entering directory.."
...
# $(eval text)eval函数表示text作为makefile的一部分,此处是定义FILES_TO_COMPILE变量
ebin/$(PROJECT).app:: $(ERL_FILES) $(CORE_FILES) $(wildcard src/$(PROJECT).app.src)
	...
	@: 以下是将应用的所有模块写入到.app.src或者.app文件
ifeq ($(wildcard src/$(PROJECT).app.src),)
	$(app_verbose) printf "$(subst $(newline),\n,$(subst ",\",$(call app_file,$(GITDESCRIBE),$(MODULES))))" \
		> ebin/$(PROJECT).app
else
	# 如果存在.app.src文件,则将该文件里面module对应的值替换为最新的模块列表
